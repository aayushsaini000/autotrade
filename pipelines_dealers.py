...
class ExtractPipeline(object):
    def __init__(self):
        ...
        self.file_name = f'AUTO_DEALER_{current_date}.csv'
        self.export_fields = [
            'Name',
            'Reviews',
            'Stars',
...
class CarguruDealerExtractPipeline(ExtractPipeline):
    def __init__(self):
        super(CarguruDealerExtractPipeline, self).__init__()
        current_date = datetime.datetime.now().date().strftime('%Y%m%d')
        self.file_name = f'CARG_DEALER_{current_date}.csv'

class AutoTraderDealerExtractPipeline(ExtractPipeline):
    def __init__(self):
        super(AutoTraderDealerExtractPipeline, self).__init__()
        current_date = datetime.datetime.now().date().strftime('%Y%m%d')
        self.file_name = f'AUTO_DEALER_{current_date}.csv'
